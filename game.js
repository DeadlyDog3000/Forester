...
const CHANGELOG = [
  ...
  { v: 15, date: "7 September 2026", title: "The woods have a sound, and a tune that goes somewhere",
    lines: [
      ...
      "And no two swings are the same any more. A colony is one axe, over and over, for an hour; played from identical numbers every time, the ear locks onto the repetition and stops hearing a forest at all. The chop, the hammer, the footfall, the quarry, the blow, the rustle and the eating all wander a few percent in pitch, length and weight. The bell, the war horn and the musket deliberately do not — those are meant to sound the same every time, which is what makes them signals rather than scenery.",
      "The menu theme was two four-bar phrases alternating: twenty-one seconds, and then the same twenty-one seconds for as long as you sat there. It has four phrases now — the theme, its answer, a lift, and a sparser settling that lets the bass speak — over three different bass figures, under an eight-phrase arrangement that brings a pad in, takes the drums away, and once in every eight lets the melody rest entirely. None of those cycles divide each other, so nothing comes round together until all three do: four minutes and ten seconds before you hear a bar exactly as you heard it before.",
      "The battle theme had one six-second riff, and a raid can last minutes. It has three now, over two basses, so a fight develops instead of stuttering.",
      "Alongside it: the infirmary, the sawmill and the recruiting post seen end-on were three plain sheds you could not tell apart. They have their cross, their saw blade and their hanging banner back, and winter coats to match.",
    ] },
  { v: 14, date: "7 September 2026", title: "Building has a screen of its own",
    lines: [
      "BUILD was a dropdown of twenty-one priced lines. A list of prices is not a decision: it never showed you what a thing looked like, what it was for, or whether you could pay for it today — so the one building you could afford looked exactly like the twenty you could not.",
      "It is a screen now. Every trade shows its own sprite, its name, its price, and a line saying what it is actually for: a well keeps more of them on their feet when plague comes, a town hall is where the colony starts running itself. The ones that cost a mark a tax day to keep say so, which is the fact that eventually bankrupts a colony and was previously buried three menus deep. They are grouped the way a builder thinks — roofs and hands, field and forge and seam, the town, faith, the wall, the ground — and a house of worship wears the creed it is about to be dedicated to and calls itself by that creed's name.",
      "What you cannot pay for today goes quiet rather than vanishing, judged against the same stores the top bar is showing you. And pressing it tells you what you are actually short of — \"2 logs short\" — instead of quoting the whole price back at you when you already have the seeds and the silver in hand.",
      "Everywhere the game explains itself, the numbers are now lifted out of the sentence: the price, the count, the hours. They are the part you have to act on, and they were exactly as loud as the rest of the words.",
    ] },
  { v: 13, date: "7 September 2026", title: "Things you were told to press that you could not see",
    lines: [
      "The third instruction the game ever gives is to press \"Deposit goods to town storage\". On an ordinary laptop that button sat three hundred and fifty pixels below the bottom of the character sheet, behind a scroll nothing advertised — the sheet appears to end at the inventory. A civilian's orders are pinned to the foot of the sheet now, and their life story scrolls behind them.",
      "Worse, step fourteen said \"in that panel, press Open Tech Tree\" — and that button was the very last thing in a panel twice as tall as the window, four hundred and ninety pixels below the bottom of the screen. Research has come out of the government panel altogether: it is RESEARCH on the bar along the bottom, one click from anywhere. What stays in the government panel is the reading of what is being researched, because that is the state of the realm.",
      "The keyboard crib along the top floated at a fixed distance that assumed the top bar was one row tall. It wraps to two rows below about fifteen hundred pixels, so on most screens the crib was underneath it — unreadable, and nothing said so. It is part of the bar now.",
      "Every label drawn over the colony — people's names, raiders, camps, a ripe field — was painted straight onto the ground with no outline. It survived summer grass by luck; four months later the ground is snow, and a winter street was a row of ghosts. They are all cut out of the ground now, the way the map's labels always were.",
      "And the first thing a new colony was ever told was \"word of your colony's wealth spreads\" — two people in a clearing, sixty marks and a burnt roof. It was announcing a rise that had not happened. It does not any more.",
      "Smaller things found by playing the opening rather than reading it: the wanderer at your gate no longer has the tutorial move on while you are still mid-sentence with him, and rebuilding the burnt cabin no longer quietly makes your brother a farmer two steps before the game tells you to make him one.",
    ] },
